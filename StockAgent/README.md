...
**Crypto mode (spot-only trading):**
```
python main.py --model MODEL_NAME --market_mode crypto
```
...
To use crypto mode, you need to:

1. Create a `data/crypto/` directory in the StockAgent folder
2. Place CSV files with OHLCV data:
   - `BTCUSDT_1h.csv` (or your chosen timeframe)
   - `ETHUSDT_1h.csv`
...
The crypto mode supports:
- Spot-only trading (no leverage, no futures, no margin)
- 24/7 trading (no market open/close)
- Fractional amounts (decimals allowed)
- Realistic fee structure (default 0.04% taker fee)
