...
import { GoogleLogin } from '@react-oauth/google'; // ĐÃ THÊM

interface LoginProps {
  onNavigate?: (page: string) => void;
}

// THÊM: Icon Google và GitHub (dưới dạng SVG component)
const GoogleIcon = () => (
...
export default function Login({ onNavigate }: LoginProps) {
  ...
  const handleGoogleLoginSuccess = async (credentialResponse: any) => {
    ...
    // Gửi idToken lên API
    // Giả sử API trả về AuthResponseDto (có accessToken, user)
    // LƯU Ý: Thay thế '/api/auth/google' bằng đường dẫn API thực tế của bạn
    try {
  ...
  // Xử lý khi nhấp nút GitHub
  const handleGitHubLogin = () => {
    setLoading(true);
    // LƯU Ý: Đảm bảo VITE_GITHUB_CLIENT_ID được định nghĩa trong tệp .env
    const githubClientId = import.meta.env.VITE_GITHUB_CLIENT_ID;
    const redirectUri = `${window.location.origin}/auth/github/callback`;
    
    // Chuyển hướng người dùng đến GitHub
          ...
          <form onSubmit={twoFARequired ? handleSubmit2FA : handleSubmit} className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <div className="relative">
                ...
                <Input
                  ...
                  required
                  disabled={twoFARequired || loading} // Thêm disabled
                />
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 gap-4">
              ...
              <GoogleLogin
                ...
                width="100%"
                logo_alignment="left"
                // Thêm thuộc tính disabled nếu cần, 
                // component này có thể không hỗ trợ trực tiếp, 
                // bạn có thể bọc nó trong 1 div và disable click
              />
